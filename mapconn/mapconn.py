...
class _PickleMixin:
    def save(self, path: Union[str, Path]) -> None:
        """Pickle the instance to a file (gzip if path ends with .gz)."""
        path = Path(path)
        if path.suffix == ".gz":
    ...
    def load(cls, path: Union[str, Path]):
        """Load a pickled instance from a file."""
        path = Path(path)
        if path.suffix == ".gz":
...
class MapConn(_PickleMixin):
    ...
    def __init__(
        ...
        if mapconn_curves is None:
            ...
            if not all(percentiles >= 0) or not all(percentiles <= 100):
                raise ValueError(
                    "All second-level column names in mapconn_curves (= percentiles) "
        ...
        if flat_connectivity_matrices is not None or flat_mappercentile_masks is not None:
            if n_parcels is None and parcel_labels is None:
                raise ValueError(
                    "if flat_connectivity_matrices or flat_mappercentile_data is provided, "
    ...
    def get_parcel_labels(
        self, flat_pairs: bool = False
    ...
    def get_connectivity_matrices(
        ...
        if flat:
            if flat_col_names:
                conn_data.columns = pd.MultiIndex.from_tuples(
                    self.get_parcel_labels(True), names=["parcelA", "parcelB"]
                )
        else:
            diagonal = np.full(
                _sym_matrix_shape_from_n_tri_elem(self._conn_data.shape[1]), fill_diagonal
            )
            conn_data = [
                vec_to_sym_matrix(conn_data.values[i, :], diagonal=diagonal)
                for i in range(conn_data.shape[0])
            ]

        return conn_data

    def get_mappercentile_masks(
        self,
        maps: Optional[Sequence[Any]] = None,
        percentiles: Optional[Sequence[float]] = None,
        flat: bool = True,
        ...
        if flat:
            ...
            if flat_col_names:
                mappct_masks.columns = pd.MultiIndex.from_tuples(
                    self.get_parcel_labels(True), names=["parcelA", "parcelB"]
                )
        else:
            mappct_masks = _mappct_flat_to_parcels(
                self._mappct_masks, parcel_labels=self._parcel_labels
            )
        maps = maps if maps is not None else self._maps
        percentiles = percentiles if percentiles is not None else self._percentiles
    ...
    def get_stats(
        ...
        if (
            mapconn_stats is None
            or recalculate
            or any(stat not in mapconn_stats.keys() for stat in stats)
        ):
            recalculate = True
        else:
            mapconn_curves = self.get_curves(maps=maps, percentiles=percentiles, ids=ids)
            if not np.array_equal(mapconn_stats[stats[0]].index, mapconn_curves.index):
                recalculate = True
            if not np.array_equal(
                mapconn_stats[stats[0]].columns.unique(),
                mapconn_curves.columns.get_level_values("map").unique(),
            ):
                recalculate = True

        if recalculate:
            mapconn_curves = self.get_curves(
                maps=maps, percentiles=percentiles, ids=ids, remove_global=False
            )
            mapconn_stats = _calc_mapconn_stats(
                mapconn_curves, stats=stats, force_dict=True, remove_global=True
            )
            self._mapconn_stats = mapconn_stats
        else:
    ...
    def get_loo(
        ...
        **kwargs,
        ...
        if loo is not None and not recalculate and return_difference:
            if (
                all(stat in loo.keys() for stat in stats)
                and all(m in loo[stats[0]].columns for m in maps)
                and all(id in loo[stats[0]].index.get_level_values("id").unique() for id in ids)
            ):
        ...
        def par_fun(what: str, loo_idx: Any) -> Dict[str, pd.DataFrame]:
            ...
            if not delta:
                mapconn_stats_loo = MapConn.from_flat_matrix(**mapconn_kwargs).get_stats(
                    force_dict=True
                )
            else:
            ...
            for stat in stats:
                if return_difference:
                    ...
                    diff[stat] = mapconn_stats_loo[
                        stat
                    ]  # that's not the difference but the stats w/o current parcel
                if what == "parcels":
                    diff[stat] = diff[stat].assign(parcel=loo_idx).reset_index(names="id")
                else:
                    diff[stat] = (
                        diff[stat]
                        .assign(parcelA=loo_idx[0], parcelB=loo_idx[1])
        ...
        diff_dfs = {
            stat: (
                pd.concat([diff_list[i][stat] for i in range(len(diff_list))], axis=0).set_index(
                    ["parcel", "id"] if what == "parcels" else ["parcelA", "parcelB", "id"]
                )
            )
            for stat in stats
        }
        ...
        if relative_difference:
            diff_dfs = {
                stat: diff_dfs[stat] / mapconn_stats[stat].rename_axis(index="id") for stat in stats
            }
    ...
    def from_flat_matrix(
        ...
        if not isinstance(flat_connectivity_matrices, (np.ndarray, pd.DataFrame)):
            raise ValueError(
                "flat_connectivity_matrices must be a numpy ndarray or pandas DataFrame"
            )
        ...
        if parcel_labels is None:
            parcel_labels = list(
                range(_sym_matrix_shape_from_n_tri_elem(flat_connectivity_matrices.shape[1]))
            )

        # store flat connectivity matrices in df
        flat_connectivity_matrices = pd.DataFrame(
            flat_connectivity_matrices, index=matrix_ids, dtype=dtype
        )
    ...
    def from_matrix(
        ...
        if isinstance(connectivity_matrices, (np.ndarray, pd.DataFrame)):
            if connectivity_matrices.ndim == 2:
                ...
                connectivity_matrices = [
                    connectivity_matrices[i, :, :] for i in range(connectivity_matrices.shape[0])
                ]
        if connectivity_matrices[0].shape[-1] != connectivity_matrices[0].shape[-2]:
            raise ValueError(
                f"connectivity_matrices must be symmetric (not shape {connectivity_matrices[0].shape}); "
        ...
        return cls.from_flat_matrix(
            flat_connectivity_matrices=conn_data_flat,
            matrix_ids=matrix_ids,
            r_to_z=r_to_z,
            map_data=map_data,
            map_data_is_pct=map_data_is_pct,
    ...
    def from_timeseries(
        ...
        if isinstance(timeseries_data, (np.ndarray, pd.DataFrame, xr.DataArray)):
            if timeseries_data.ndim == 1 or timeseries_data.ndim > 3:
                ...
                timeseries_data = [
                    np.array(timeseries_data[i, :, :]) for i in range(timeseries_data.shape[0])
                ]

        # z standardize per parcel (axis=0 = time axis)
        if zscore:
            timeseries_data = [
                (timeseries_data[i] - np.mean(timeseries_data[i], axis=0))
                / np.std(timeseries_data[i], axis=0)
        ...
        if isinstance(connectivity_estimator, str):
            if connectivity_estimator.lower() in ["pearson", "correlation", "corr"]:
                connectivity_estimator = {
                    "method": "empiricalcovariance",
                    "kind": "covariance",
                    "normalize": True,
                }
            elif connectivity_estimator.lower() in [
                "partial_pearson",
                "partial_correlation",
                "partial_corr",
            ]:
            ...
            if not isinstance(connectivity_estimator, dict):
                raise ValueError(
                    "connectivity_estimator must be a dictionary or a predefined string"
                )
            else:
                if any(
                    k not in connectivity_estimator.keys() for k in ["method", "kind", "normalize"]
                ):
                    raise ValueError(
                        "connectivity_estimator dictionary must contain 'method', 'kind', and 'normalize'"
                    )
        ...
        conn_data = Parallel(n_jobs=n_jobs)(
            ...
            for i in tqdm(
                ...
                disable=not verbose,
                desc="Calculating connectivity matrices",
            )
        )

        # run MAPCONN.from_matrix()
        return cls.from_matrix(
            connectivity_matrices=conn_data,
            matrix_ids=timeseries_ids,
            r_to_z=connectivity_estimator["normalize"],
            map_data=map_data,
            map_data_is_pct=map_data_is_pct,
            flat_mappercentile_masks=flat_mappercentile_masks,
            conn_aggregation=conn_aggregation,
...
class MapConnNull(_PickleMixin):
    """
    ...
    def __init__(
        ...
        # precompute
        if get_results:
            self._ensure_results(include_pvalues=compute_pvalues, include_delta=compute_delta)
    ...
    def get_null_curves(
        ...
        obs_sel = self._mapconn_instance.get_curves(
            maps=maps, percentiles=percentiles, ids=ids, remove_global=False
        )
        ...
        if return_df:
            if remove_global:
                ...
                null = [
                    pd.DataFrame(arr, index=obs_sel.index, columns=obs_sel.columns) for arr in null
                ]
        return null

    def get_null_curves_dist(
        self,
        maps: Optional[Sequence[Any]] = None,
        percentiles: Optional[Sequence[float]] = None,
        ids: Optional[Sequence[Any]] = None,
        ...
        if self._mapconn_null_curves_dist is None or recalculate:
            ...
            if not dist_stats_from_mean:
                raise NotImplementedError(
                    "dist_stats_from_mean must be True, distribution stats can "
        ...
        dist_stats = (
            ["count", "mean", "std", "min"]
            + [f"{q*100}%".replace(".0", "") for q in dist_stats_quantiles]
            + ["max"]
        )
    ...
    def get_null_stats(
        ...
        List[pd.DataFrame], Dict[str, List[pd.DataFrame]], pd.DataFrame, Dict[str, pd.DataFrame]
    ]:
        ...
        if (
            mapconn_null_stats is None
            or recalculate
            or (any(stat not in mapconn_null_stats.keys() for stat in stats))
        ):
            recalculate = True
        elif mapconn_null_stats is not None:
            # get mapconn curves for reference
            mapconn_curves = self._mapconn_instance.get_curves(
                maps=maps, percentiles=percentiles, ids=ids, remove_global=True
            )
            # check if all stats and data are available
            if not all(stat in mapconn_null_stats.keys() for stat in stats):
                recalculate = True
            if not np.array_equal(mapconn_null_stats[stats[0]][0].index, mapconn_curves.index):
                recalculate = True
            if not np.array_equal(
                mapconn_null_stats[stats[0]][0].columns.unique(),
                mapconn_curves.columns.get_level_values("map").unique(),
            ):
                recalculate = True
        ...
        if recalculate:
            mapconn_null_curves = self.get_null_curves(
                maps=maps, percentiles=percentiles, ids=ids, remove_global=False, inverted=inverted
            )
            mapconn_null_stats = []
            for mapconn_null_curves_i in mapconn_null_curves:
                mapconn_null_stats.append(
                    _calc_mapconn_stats(
                        mapconn_null_curves_i,
                        stats=stats,
                        force_dict=True,
            ...
            mapconn_null_stats = {
                stat: [null[stat] for null in mapconn_null_stats]
        ...
        if multilevel_index:
            mapconn_null_stats = {
                stat: pd.concat(
                    mapconn_null_stats[stat],
                    axis=0,
                    keys=range(len(mapconn_null_stats[stat])),
                    names=["null", "id"],
                )
                for stat in stats
            }
    ...
    def get_delta_null_stats(
        ...
        for stat in stats:
            null_stats_delta[stat] = [
                null_stats_original[stat][i] - null_stats_inverted[stat][i] for i in range(n_nulls)
            ]
    ...
    def get_null_stats_dist(
            ...
            for stat in stats:
                ...
                if dist_stats_from_mean:
                    ...
                    for i, id in enumerate(ids):
                        n = null[:, i, :]
                        null_dist[stat][id] = pd.DataFrame(n, columns=maps).describe(
                            percentiles=dist_stats_quantiles
                        )
        ...
        dist_stats = (
            ["count", "mean", "std", "mad", "min"]
            + [f"{q*100}%".replace(".0", "") for q in dist_stats_quantiles]
            + ["max"]
        )
        null_dist = {
            stat: mapconn_null_stats_dist[stat].loc[
                dist_stats if dist_stats_from_mean else (slice(None), dist_stats), maps
            ]
            for stat in stats
        }
    ...
    def get_delta_null_stats_dist(
        ...
        if null_stats_delta is None or recalculate:
            ...
            null_stats_dist = self.get_null_stats_dist(
                ...
                percentiles=percentiles,
                ids=ids,
                dist_stats_from_mean=dist_stats_from_mean,
                dist_stats_quantiles=dist_stats_quantiles,
                recalculate=recalculate,
                force_dict=True,
            )
        ...
        null_dist = {
            stat: null_stats_delta[stat].loc[
                dist_stats if dist_stats_from_mean else (slice(None), dist_stats), maps
            ]
            for stat in stats
        }
    ...
    def get_matrix_sac(
        self, distmat: Optional[np.ndarray] = None, ids: Optional[Sequence[Any]] = None, **kwargs
    ) -> pd.DataFrame:
        """
        Passed through to the original mapconn instance. See MapConn.get_matrix_sac() for details.
    ...
    def get_pvalues(
        ...
        if pvalues is None or recalculate or (any(stat not in pvalues.keys() for stat in stats)):
            ...
            if p_from_mean:
                ...
                if not np.array_equal(pvalues[stats[0]].index, mapconn_stats[stats[0]].index):
                    recalculate = True
            if not np.array_equal(pvalues[stats[0]].columns, mapconn_stats[stats[0]].columns):
                recalculate = True
        ...
        if recalculate:
            ...
            for stat in set(stats).intersection(set(mapconn_stats.keys())):
                ...
                pvalues[stat] = pd.DataFrame(
                    np.reshape(pvalues_stat, (len(ids), len(maps))), columns=maps, index=ids
                )
    ...
    def get_delta_pvalues(
        ...
        if not p_from_mean:
            raise NotImplementedError(
                "p_from_mean must be True, p-values for delta stats can "
                "currently only be calculated for mean across subjects"
            )
        ...
        null_curves_inverted = Parallel(n_jobs=n_jobs)(
            ...
            for map_data_null_i in tqdm(
                map_data_null_inverted,
                disable=not verbose,
                desc="Calculating inverted null mapconn curves",
            )
        )
        # save
        ...
        for stat in stats:
            pvalues[stat] = [
                null_to_p(
                    ...
                    null_array=[
                        null_stats_delta[stat][i].values[:, i_m].mean() for i in range(n_nulls)
                    ],
                    tail=tail,
                    fit_norm=False,
                )
            ...
            pvalues[stat] = pd.DataFrame(
                np.array(pvalues[stat])[np.newaxis, :], columns=maps, index=["mean"]
            )
        ...
        for stat in stats:
            for metric in metrics:  # TODO: delta?
                ...
                if level == "individual":
                    ...
                    df.append(
                        df_summary.loc[(stat, metric, "val", slice(None)), :].reset_index()
                        # .assign(variable="val")
                        .set_index(["curve_stat", "metric", "variable", "id"])
                    )
                    ...
                    if metric in ["original", "delta"]:
                        if metric == "original":
                            ...
                            try:
                                ...
                                continue
                        df.append(
                            tmp.reset_index()  # makes columns: id and variable
                            .assign(
                                variable=lambda x: "null_" + x.variable,
                                curve_stat=stat,
                                metric=metric,
                            )
                            .set_index(["curve_stat", "metric", "variable", "id"])
                ...
                # group
                    ...
                    # results from summary
                    df.append(df_summary.loc[(stat, metric, slice(None), slice(None)), :])
                    ...
                    if metric in ["original", "delta"]:
                        ...
                        df.append(
                            tmp.assign(
                                variable=lambda x: "null_" + x.index, curve_stat=stat, metric=metric
                            ).set_index(["curve_stat", "metric", "variable"])
                        )
            ...
            for metric in metrics:
                ...
                if level == "group":
                    ...
                    try:
                        ...
                        continue
                    df.loc[(stat, metric, "mean_z"), :] = (
                        mean.values - null_mean.values
                    ) / null_std.values
                    df.loc[(stat, metric, "mean_rz"), :] = (mean.values - null_med.values) / (
                        1.4826 * null_mad.values
                    )
                    ...
                    df = pd.concat(
                        [
                            df,
                            ((val - null_mean.values) / null_std.values).rename(
                                index={"val": "val_z"}
                            ),
                            ((val - null_med.values) / (1.4826 * null_mad.values)).rename(
                                index={"val": "val_rz"}
                            ),
                        ]
                    )
    ...
    def _ensure_results(
        ...
        if stats is None:
            stats = "auc"
        logger.info("Pre-computing stats")
        self.get_stats(stats=stats)
        if include_pvalues:
            combinations = list(product([True, False], [True, False]))
            for i, (p_from_mean, inverted) in enumerate(combinations):
                level = "group" if p_from_mean else "individual"
                ...
                try:
                    ...
                    pass
        logger.info("Pre-computing null curve distributions")
        self.get_null_curves_dist()
        logger.info("Pre-computing null stats distributions")
        ...
        if include_delta:
            try:
                logger.info("Pre-computing delta p-values")
                self.get_delta_pvalues(stats=stats, verbose=verbose)
                logger.info("Pre-computing delta null stats distributions")
    ...
    def drop_nulls(
        self,
        ensure_results: bool = True,
        keep_null_stats: bool = False,
        stats: Optional[Union[str, Sequence[str]]] = None,
    ) -> None:
        """
        ...
        if stats is None:
            stats = "auc"
        if ensure_results:
            self._ensure_results(include_pvalues=True, include_delta=True, stats=stats)
    ...
    def save(
        self, path: Union[str, Path], drop_nulls: bool = True, ensure_results: bool = True
    ) -> None:
        """Pickle the instance to a file, optionally dropping null data first."""
        if drop_nulls:
    ...
    def from_mapconn(
        ...
        **kwargs,
        ...
        if parcellation is None and distmat is None and map_data_null is None:
            raise ValueError(
                "Either parcellation or distance matrix or map_data_null must be provided"
            )
        ...
        mapconn_null_curves = Parallel(n_jobs=n_jobs)(
            ...
            for map_data_null_i in tqdm(
                map_data_null, disable=not verbose, desc="Calculating null mapconn curves"
            )
        )

        # return
        return cls(
            mapconn_instance=mapconn_instance_input,
...
def calculate_mapconn(
    ...
    if conn_data_flat.ndim != 2:
        raise ValueError(
            "conn_data must be a 2D array (flattened lower triangle of matrix/matrices)"
        )
    if map_data is not None:
        ...
        if conn_data_flat.shape[1] != _n_sym_matrix_tri_elem_from_shape(map_data.shape[1]):
            raise ValueError(
                "flat_connectivity_matrices and map_data must have corresponding number of columns. "
        ...
        if conn_data_flat.shape[1] != mappct_masks_flat.shape[1]:
            raise ValueError(
                "flat_connectivity_matrices and flat_mappercentile_data must have same number of columns (=elements)"
            )
    else:
    ...
    mapconn = Parallel(n_jobs=n_jobs)(
        ...
        for map_idx in tqdm(
            range(mappct_masks_flat_arr.shape[0]),
            disable=not verbose,
            desc="Calculating mapConn curves",
        )
    )
    ...
    if return_df:
        mapconn = pd.DataFrame(
            mapconn,
            index=(
                flat_connectivity_matrices.index
                if isinstance(flat_connectivity_matrices, pd.DataFrame)
                else None
            ),
            columns=(
                mappct_masks_flat.index if isinstance(mappct_masks_flat, pd.DataFrame) else None
            ),
            dtype=dtype,
        )
