...
def _calc_mapconn_stats(mapconn_curves: pd.DataFrame, stats: Union[str, Sequence[str]] = "all",
                        remove_global: bool = True, force_dict: bool = False
...
def auc(curve: Union[pd.Series, np.ndarray], percentiles: Sequence[float], square_curve: bool = False) -> float:
...
def poly(curve: Union[pd.Series, np.ndarray], percentiles: Sequence[float], degree: int = 2) -> float:
