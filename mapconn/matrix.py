...
def _get_matrix_estimator(
    method: str = "EmpiricalCovariance",
    kind: str = "covariance",
    normalize: bool = True,
    ...
    **kwargs: Any,
    ...
    # other options: use sklearn
        ...
        if normalize:
            norm_fun = (
                _covariance_to_pearson
                if matrix_attr == "covariance_"
                else _precision_to_partial_pearson
            )
        else:
        ...
        def matrix_fun(timeseries: np.ndarray) -> np.ndarray:
            ...
            matrix = getattr(estimator(**kwargs).fit(np.array(timeseries)), matrix_attr)
        ...
        if verbose:
            if method.lower() == "empiricalcovariance" and kind == "precision" and normalize:
                ...
                logger.info(
                    "Estimating %s%s matrix using sklearn - %s",
...
def _sym_matrix_shape_from_n_tri_elem(num_elements: int, discard_diagonal: bool = True) -> int:
    ...
    if discriminant < 0:
        raise ValueError(
            "No real solutions. The number of elements does not correspond to a valid symmetric matrix."
        )
...
def _vectorize_sym_matrices(
    sym_matrices: Union[np.ndarray, Sequence[np.ndarray]], discard_diagonal: bool = True
) -> np.ndarray:
    """Vectorize a list/array of symmetric matrices."""
    return np.array(
        [sym_matrix_to_vec(np.asarray(m), discard_diagonal=discard_diagonal) for m in sym_matrices]
    )


# spatial autocorrelation of a connectivity matrix
def matrix_sac(
    matrix: np.ndarray, distmat: np.ndarray, discretization: Optional[Union[str, float]] = None
) -> np.ndarray:
